...
g       = 9.81
k_motor = 0.9
Load_Ctroll = Load_Transport_Controller_Disturbance_Removal()
PAR = collections.namedtuple('DI_paramteres',['M','m','L','g','k_motor','Controller'])
parameters = PAR(M,m,L,g,k_motor,Load_Ctroll)
...
yd0 = traj_des(t0)
U   = parameters.Controller.output(y0, yd0)
r.set_initial_value(y0, t0).set_f_params(U,parameters)
...
while r.successful() and r.t < t1:
    ...
    Estimate.append(Load_Ctroll.k_Motor_est)
...
#    Yvalue.append(numpy.linalg.norm(xM - x))
    ...
    # desired position and velocity for LOAD
    yd = traj_des(r.t + dt)
    U  = parameters.Controller.output(r.y, yd)
    r.set_f_params(U,parameters)    
